...
The personalised dream version of the product (deliberately NOT in scope now):
- Read a user's Steam library on every visit so results stay current with new purchases
- Run a small AI agent in the background that looks up those titles and tags each one into the mood categories
- Let users define their own custom categories to steer how the AI sorts their games
